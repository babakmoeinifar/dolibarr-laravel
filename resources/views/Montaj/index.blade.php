                        ...
                        <div class="modal-body">
                            <form action="{{route('sales.store')}}" method="post">
                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group col-md-6">
                                        <label for="exit_date">تاریخ خروج</label>
                                        <input type="text" class="form-control" id="exit_date" name="exit_date"
                                               required>
                                    </div>
                                </div>
                                <br>
                                <label for="device_id">مدل دستگاه</label>
                                <div class="text-danger" id="deviceError" style="display: none;">
                                    لطفا حداقل یک دستگاه را انتخاب کنید
                                </div>
                                <div class="row">
                ...
                <div class="row">
                    ...
                    @foreach($groupedSales as $groupedSale)
                        @php
                            ...
                            $isUrgent = $daysUntilExit < 5;
                            $saleIds = $groupedSale->pluck('id')->join(',');
                        @endphp
                        <div class="col-md-3 mb-3">
                            <div class="card {{ $isUrgent ? 'bg-danger text-white' : 'bg-light' }} position-relative">
                                @if($groupedSale[0]->created_at == today())
                                <span class="badge bg-success text-white position-absolute" style="top: 10px; left: 10px; font-size: 0.75rem;">جدید</span>
                                @endif
                                <div class="card-body">
                                    ...
                                    <div class="list-unstyled text-end">
                                        @foreach($groupedSale as $sale)
                                            <div class="d-flex justify-content-between">
                                                <span style="font-family: 'Helvetica'">{{ $sale->device->en_name }}</span><b>{{ $sale->qty }}</b>
                                            </div>
                                        @endforeach
                                    </div>
                                    <br>
                                    <span class="badge bg-warning text-dark mt-2 d-flex justify-content-around" style="font-size: 16px;">
            {{ $daysUntilExit }} روز مانده
        </span>
                                    <div class="border-top pt-2 d-flex justify-content-between mt-2">
                                        <a href="{{ route('sales.toggleStatus', ['status' => 'done', 'ids' => $saleIds]) }}"
                                           ...
                                           data-bs-toggle="tooltip" data-bs-placement="top" title="مسٔول انبار">خارج
                                            شد</a>
